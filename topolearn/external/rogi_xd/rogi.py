...
def estimate_max_dist(X: np.ndarray, metric: Metric) -> float:
    ...
    if metric == Metric.EUCLIDEAN:
        ...
        d_max = 2.0
    elif metric == Metric.ROGERSTANIMOTO:
        d_max = np.max(range_)
...
def calc_distance_matrix_fps(
    fps: Sequence[ExplicitBitVect], metric: Metric = Metric.ROGERSTANIMOTO
) -> np.ndarray:
    logger.info("Computing distance matrix...")

    if metric == Metric.ROGERSTANIMOTO:
        ...
        raise ValueError(
            ...
            f" got: {metric.value}. expected one of: (Metric.ROGERSTANIMOTO, Metric.DICE)"
        )
...
def calc_distance_matrix(
    ...
    NOTE: see :func:`~rogi_xd.rogi.rogi` for details on the following arguments: `xs`, `metric`,
    ...
    if isinstance(xs, np.ndarray):
        ...
        metric = Metric.get(metric) if metric is not None else Metric.ROGERSTANIMOTO
        ...
        metric = Metric.get(metric) if metric is not None else Metric.ROGERSTANIMOTO
        D = calc_distance_matrix_fps(fps, Metric.ROGERSTANIMOTO)
    else:
        raise TypeError(
            "arg 'xs' must be of type `np.ndarray` | `Sequence[ExplicitBitVect]` | `Sequence[str]!`"
        )
...
def rogi(
    ...
    metric : Union[str, Metric, None], default=None
        ...
        2) `Sequence[ExplicitBitVect]`: `Metric.ROGERSTANIMOTO`
        3) `Sequence[str]`: `Metric.ROGERSTANIMOTO`
    ...
    -------
    RogiResult
        the result of the calculation. See :class:`~rogi_xd.utils.rogi.RogiResult` for more details
    """
    y = np.array(y)
